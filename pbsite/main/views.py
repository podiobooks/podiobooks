...
def title_search(request, keywords=None):
    ...
    if keywords != None:
        if settings.SEARCH_PROVIDER == 'SPHINX':
            search_results = Title.search.query(keywords).set_options(passages=True, passages_opts={'before_match':"<font color='red'>",'after_match':'</font>','chunk_separator':' ... ','around':6,}) 
        else:
