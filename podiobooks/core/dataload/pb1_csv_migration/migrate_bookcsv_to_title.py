...
def get_or_create_contributor(contributor_name):
    ...
    contributor_name = contributor_name.strip().replace('  ', ' ').replace('\\', '').replace('&apos;', '\'').replace(
        'Theater', 'Theatre').replace('J. C.', 'J.C.').replace('J. A.', 'J.A.').replace('J. J.', 'J.J.').replace('J. T.'
        , 'J.T.').replace('J. P.', 'J.P.').replace('JP', 'J.P.').replace('I G', 'I.G.')
    try:
    ...
    contributor, created = Contributor.objects.get_or_create(display_name__iexact=contributor_name,
        defaults={'display_name': contributor_name, 'slug': slugify(contributor_name), 'first_name': first_name_guess,
                  'middle_name': middle_name_guess.replace(',', ''),
                  'last_name': last_name_guess.replace(',', '').replace(' Ph.D.', '')})
    return contributor
...
def create_titles_from_book_rows(title_list):
    ...
    for row in title_list:
        ...
        if (int(row['Enabled']) == 1 and int(row['Standby']) == 0):
            ...
            # Contributors
            contributor_list = contributor_translation.translate_contributor(
                row['Authors']) #Manual Contributor Lookup Translation
