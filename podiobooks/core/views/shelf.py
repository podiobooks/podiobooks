...
class FilteredShelf(View):
    ...
    def top_rated_by_author(self, author=None):
        ...
        """
        toprated_title_list = Title.objects.prefetch_related("titlecontributors", "titlecontributors__contributor", "titlecontributors__contributor_type").filter(display_on_homepage=True, deleted=False, promoter_count__gte=20).order_by('-date_created')
    ...
    def featured_by_category(self, category=None):
        ...
        """
        featured_title_list = Title.objects.prefetch_related("titlecontributors", "titlecontributors__contributor", "titlecontributors__contributor_type").filter(display_on_homepage=True, deleted=False).order_by('-date_created').all()
