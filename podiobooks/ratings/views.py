...
class RateTitleView(View):
    ...
    def post(self, request, pk=None, up=True):
        ...
        # Also, since the cookies are sent back and forth on each request, the more you vote, the slower the site
...
def already_voted(request, title_pk):
    title_pk = str(title_pk)
    vote_cookie = request.get_signed_cookie('v', False)
    if vote_cookie:
        vote_cookie_list = vote_cookie.split(',')
        if title_pk in vote_cookie_list:
            return True

    ip_title_list = cache.get(request.META['REMOTE_ADDR'])
    if ip_title_list:
        if title_pk in ip_title_list:
            return True

    return False


class RatingsTestView(TemplateView):
    template_name = "ratings/test.html"